document.addEventListener('DOMContentLoaded', function() {
    function createAlertTimeEntry(time = '', sound = 'alarm.mp3', volume = 0.5) {
        ...
        container.innerHTML = `
            ...
            <select class="alert-sound">
                ...
                <option value="alert5.mp3" ${sound === 'alert5.mp3' ? 'selected' : ''}>Alert5</option>
            ...
            <div class='volume-control'>
                <label>Volume: <span class='volume-display'>${Math.round(volume * 100)}%</span></label>
                <input type='range' class='volume-slider' min='0' max='1' step='0.01' value='${volume}'>
            </div>
            <span class="remove-alert-time">Remove</span>
        `;

        container.querySelector('.volume-slider').addEventListener('input', function(e) {
    ...
    chrome.storage.local.get(['alertTimes'], function(result) {
        ...
        if (alertTimes.length === 0) {
            ...
            alertTimes.forEach(({ time, sound, volume }) => createAlertTimeEntry(time, sound, volume));
        }
    });
